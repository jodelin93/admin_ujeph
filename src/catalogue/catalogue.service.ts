...
export class CatalogueService extends AbstracService {
  constructor(
    @InjectRepository(Catalogue) private catalogueRepo: Repository<Catalogue>,
    ...
    if (faculte && teacher && courses) {
      ...
      if (catalogueSaved) {
        throw new BadRequestException(
          'ce cours a deja ete attribuer a cette faculte pour ce meme semestre',
        );
      }
      return await this.catalogueRepo.save(catalogue);
    } else {
      throw new BadRequestException(
        'toutes les ressources doivent disponibles dans la base',
      );
    }
  }
